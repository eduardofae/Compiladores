...
lst_ids : lst_ids ';' TK_IDENTIFICADOR 
        | TK_IDENTIFICADOR;
...
header_params : lst_parameters 
              | ;
lst_parameters : lst_parameters ';' parameter 
               | parameter;
...
commands : lst_commands 
         | ;
lst_commands : lst_commands command ',' 
             | command ',';
...
eq_exp : eq_exp equality ineq_exp 
       | ineq_exp;
equality : TK_OC_EQ 
         | TK_OC_NE;

/* Expressão de inequidade */
ineq_exp : ineq_exp inequality sum_exp 
         | sum_exp;
inequality : TK_OC_GE 
           | TK_OC_LE 
           | '<' 
...
sum_exp : sum_exp sum prod_exp 
        | prod_exp;
sum : '+' 
    | '-';

/* Expressões de produto e divisão */
prod_exp : prod_exp prod unary_exp 
         | unary_exp;
prod : '*' 
     | '/' 
     | '%';
